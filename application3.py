...
def main():
    ...
    x0, y0, width, height = 120, 120, 120, 320
    ...
    while True:
        ...
        if showMask:
            window[y0:y0+width, x0:x0 +
                   height] = cv2.cvtColor(roi, cv2.COLOR_GRAY2BGR)
        ...
        if takingData:
            ...
            cv2.putText(window, 'Prediction: %s' %
        ...
        # adjust the position of window
            ...
            x0 = min((x0 + 5, window.shape[1]-height))
